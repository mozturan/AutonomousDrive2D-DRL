...
class RacetrackEnv(AbstractEnv):
    ...
    def _reward(self, action: np.ndarray) -> float:
        ...
        """
        * Reward = ( Lane centering [0,1] * Speed [0,1] + Action [0,0.3] ) * On_Road
        """
        reward = rewards["lane_centering_reward"] * rewards["speed_reward"]
        reward = reward + rewards["action_reward"] * self.config["action_reward"]
    ...
    def _rewards(self, action: np.ndarray) -> Dict[Text, float]:
        ...
        return {
            ...
            "speed_reward": self.gaussian_reward(x = self.vehicle.speed)
        }

    def gaussian_reward(self, x, target_value =10, spread=3.0):
        """
        Calculate the reward based on a Gaussian function.
        The reward is 1 when x is equal to the target_value and decreases smoothly as x deviates from the target.
        """
        return math.exp(-(x - target_value)**2 / (2 * spread**2))
